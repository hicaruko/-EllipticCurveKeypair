...
class FlutterEllipticCurveKeyPair {
  ...
  static Future<EccResultModel> ellipticCurveKeyPairSigning(
      {required String message, String? language, String? alias}) async {
    Map arguments = {
      "message": message,
      "language": language,
      "alias": alias,
  ...
  /// return [EccResultModel] with [Exception] if fail
  static Future<EccResultModel> ellipticCurveKeyPairPublicKey(
      {String? alias}) async {
    Map arguments = {
      "alias": alias,
    };
    ...
    try {
      final String? publicKey = await _channel.invokeMethod(
          'ellipticCurveKeyPairPublicKey', arguments);
      EccResultModel result = EccResultModel(
        data: publicKey,
        success: true,
  ...
  /// return [EccResultModel] with [Publickey] if success
  /// return [EccResultModel] with [Exception] if fail
  static Future<EccResultModel> ellipticCurveKeyPairPublicKeyPem(
      {String? alias}) async {
    Map arguments = {
      "alias": alias,
    };
    ...
    try {
      final String? publicKey = await _channel.invokeMethod(
          'ellipticCurveKeyPairPublicKeyPem', arguments);
      EccResultModel result = EccResultModel(
        data: publicKey,
        success: true,
        description: "Public key recieved successfully",
